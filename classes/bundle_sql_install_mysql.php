            ...
            if ($this->_mysql_dirs[$bundle->name]) {
                ...
                foreach ($files as $file) {
                    ...
                    if ($sql_script->safe_to_run($bundle->name, $file, self::DIALECT)) {
                        $cmd = 'mysql -h ' . $this->data_source->get_host(true)['host'] . ' -u ' . $this->data_source->get_host(true)['username'] . ' -p' . $this->data_source->get_host(true)['password'] . ' -D ' . $this->data_source->get_host(true)['schema'] . ' --default-character-set=' . $this->setting('mysql.default_character_set') . ' < ' . $file;
                        exec($cmd);
